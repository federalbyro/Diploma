...
def resolve_label_path(input_path: Path, label_dir: Path) -> Path:
    return label_dir / f"{input_path.stem}_label_ru.csv"
...
def load_reference_texts(reference_file: Path) -> list[str]:
    df = _read_csv_with_fallback(reference_file, header=None, names=["reference_ru"])
    df["reference_ru"] = df["reference_ru"].fillna("").astype(str).str.strip()
    return df["reference_ru"].tolist()
